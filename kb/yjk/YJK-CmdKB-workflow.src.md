...
### 2.4 步④ 布基础(默认筏板) → 覆写 Jccad_0.ydb `[脚本可headless]`

> **基础选型: 默认筏板基础**(`JcRaftSlab_Def`/`JcRaftSlab_App`)。软土深厚 / 高层重载 / 抗浮时改**桩基础**(`JcPile_*` + `JcDais_*` 承台, 见 patterns 范式 2C)。两种基础都覆写同一 `Jccad_0.ydb`、GFE 导入流程完全一致——**无论筏板还是桩, `Jccad_0.ydb` 都必须产出, 并随 dtlmodel/dtlCalc 一起交给 GFE**(缺 Jccad_0 则 GFE 端无任何基础, Raft/桩都为空)。
