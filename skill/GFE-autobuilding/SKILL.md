...
- **★YJK→GFE 完整流程(2026-06-15 地铁车站实测跑通, 严格按此顺序; 详见记忆 `reference_gfe_import_yjk_para`)**:
  - **步0 建 IPC链接(用户手动, 必提示)**: 用户开 YJK → 打开本工程(dtlmodel) → YJK 命令行输一次 `yjksipccontrol`。之后外部 `YJKSControl.RunCmd` 才能驱动它。换工程/重开后须重输。
  - **步1 dtlmodel.ydb(headless 全自动)**: YJKAPI `CreateYDB` 建上部结构。**底板用筏板基础的(地铁车站/地下室)→上部就别建底板层**(柱墙底落 z=0, 底板留给步5 GFE 自动生成)。
  - **步2 前处理及计算(IPC, 只算!别接导出)**: `RunCmd` 序列 `yjk_repairex/yjk_save/yjk_setlayersupport/yjkspre_genmodrel/yjktransload_tlplan/_tlvert/SetCurrentLabel IDSPRE_ROOT/yjkdesign_dsncalculating_all/yjk_save`(秒级跑完=对应GUI"上部结构计算→前处理及计算")。**必须先算完, 再单独导出——别把计算和导出塞一个脚本连着 RunCmd(会出问题)**。
  - **步3 导出 dtlCalc.ydb**: 算完用 `yjk_exportydb`(=GUI 工具栏蓝箭头"导出→YJK对外接口文件"按钮)导出, 默认落到工程 `施工图\` 子目录。⚠**此命令弹「导出设置」对话框, 纯 IPC RunCmd 会卡死等点确定**——命令流过不去, 需 pywinauto 自动操作对话框(填名+选"YJK对外接口文件"+点确定) 或 用户手动点。(`yjk_exportydb_SHTH` KB 标"不弹窗"但实测照样弹, 别信)
  - **步4 Jccad_0.ydb**: `jccad_read`(IPC)产初始 Jccad_0(工程主目录)。把它**复制进 `施工图\`**, 让 dtlmodel+dtlCalc+Jccad_0 三件齐聚一个目录(GFE import 目录就指这个)。⚠**别走 headless `JcDataFunc.JcRaftSlab_Def/App` 布筏板**: 无 Assign/Commit、ReadJcYdb→CreateYDB 往返丢数据, 实测死路——筏板一律走步5 GFE 自动生成。
  - **步5 GFE import_yjk(全自动)**: `io.get_current().import_yjk(三件ydb目录, yjk_para, ['',''], True, '', False)` + `document.set_application_by_ui()`。**yjk_para(43整数)第8位=2 → "自动生成基础模型"**(GFE 按车站底部自动生成筏板, 无视空 Jccad_0; 第8位=0 则"读取基础模型", 读空 Jccad_0 无筏板); **第13位 = 自动生成筏板厚 mm**(如1000)。其余位: 11=筏板周长m / 14=超出外轮廓mm / 9=最大板面积 / 18=维度3 / 22=地下室点合并 / 26·27=梁夹角·最小板内角 / 33=最小板面积。验证: gset 出 `AllRaft/floor0-AllRaft/Raft_Conc<厚>`、sect 出同名筏板截面、geom 出 `BasementBoundary`。⚠ yjk_para 数值位可跨案例参考, 但**别整段照抄不同结构**(查 paths ch10/11/15)。
