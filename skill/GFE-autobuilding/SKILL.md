...
- **三ydb(实测,GFE导入缺一不可)**：`dtlmodel.ydb`(YJKAPI CreateYDB headless 产) + `dtlCalc.ydb`(前处理计算产) + `Jccad_0.ydb`(基础;**默认筏板**,Raft 来自此,去掉则无底板;软土/重载可改桩基 JcPile+承台,见 ⓪YJK KB patterns 2C)。后两者需 **YJK 软件运行中**——可命令驱动不必纯手点: 计算序列 `yjk_setlayersupport/yjkspre_genmodrel/yjktransload_tlplan/_tlvert/yjkdesign_dsncalculating_all`(出 dtlCalc); `jccad_read` 重新读取上部数据(出初始 Jccad_0) + `JcDataFunc.JcRaftSlab_Def/App` 布筏板。详见 ⓪YJK KB `-workflow`/`-patterns`。
