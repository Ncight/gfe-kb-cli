...
- **★YJK→GFE 完整流程(2026-06-15 地铁车站【二次实测重写】, 推翻旧"命令流 import_yjk 一把梭"; 详见记忆 `reference_gfe_import_yjk_para`)**。分工: **AI 命令流做步0-4, 用户手动做步5-6 GUI**(命令流 import_yjk 不可靠), AI 接手步7。
  - **步0 建 IPC链接(用户手动, 必提示)**: 用户开 YJK → 打开本工程 → 命令行输一次 `yjksipccontrol`。之后 `YJKSControl.RunCmd` 才能驱动。换工程/重开须重输；**反复连接会污染会话致 RunCmd 卡死, 重输一次即恢复**。
  - **步1 dtlmodel.ydb(headless 全自动)**: YJKAPI `CreateYDB` 建上部结构。**底板用筏板基础的(地铁车站/地下室)→上部别建底板层**(柱墙底落 z=0, 底板留给步6 导入时自动生成筏板)。
  - **步2 前处理及计算(IPC)**: `RunCmd` 序列 `yjk_save(★暖场)/yjk_repairex/yjk_setlayersupport/yjkspre_genmodrel/yjktransload_tlplan/_tlvert/SetCurrentLabel IDSPRE_ROOT/yjkdesign_dsncalculating_all/yjk_save`。⚠**连接后第一条 RunCmd 易卡死, 必须 `yjk_save` 暖场起手**(SetCurrentLabel 当第一条会卡)。**指定 `config.Pid=<yjks进程PID>`** 避免 Pid=-1 弹"选择 yjks.exe 进程"框。
  - **步3 Jccad_0.ydb(IPC)**: `jccad_read` 产初始 Jccad_0。⚠**无需布筏板**——jccad_read 产的初始基础(只有柱墙荷载、无筏板几何)就够, 筏板步6 导入时自动生成。(headless `JcDataFunc.JcRaftSlab_Def/App` 布筏板仍死路, 别走)
  - **步4 三件归拢 `施工图\`**: dtlmodel.ydb + Jccad_0.ydb 复制进工程 `施工图\` 子目录。⚠**GFE 导入目录必须是 `施工图\`**(放 `YDB\` 等别处会读取错位→导入全空 gset=0)。dtlCalc 见步5。
  - **步5 dtlCalc.ydb(★最易错, 用户手动导出)**: dtlCalc 是**计算结果数据**(工况内力/位移/配筋), 必须用 **`yjkdesign_dsnExportDB`**(YJK 标题栏导出按钮) → 弹「导出dsnModel」对话框 → **手动勾全部 6 个计算项**(导出工况振型内力/位移·面单元内力/位移·体单元内力/位移; 默认只勾"导出基础数据") → 确定 → 产 `中间数据\dsnModel.ydb`(**272 表**) → **改名 dtlCalc.ydb** 放 `施工图\`。
    - ⚠**绝不能用 `yjk_exportydb`** 当 dtlCalc: 它默认名就是 dtlmodel、产 **dtlmodel 副本(79表, schema 与 dtlmodel 全同)**, GFE 读它刷 **17× "the column name of sqlite is invalid"** 再 **0xC0000374 堆损坏崩溃(exit 116)**。
    - ⚠**6 个计算项必须勾全**: 只默认基础数据→**32表**精简版、不含计算内力, 也不对。
    - **SQLite 表数鉴别**: dtlmodel/假dtlCalc=**79表**; 精简版=**32表**; **真 dtlCalc(dsnModel)=272表**。可用 `sqlite_master` 查表数自检。
  - **步6 GFE 导入(★用户手动 GUI, 不走命令流)**: 用户在 GFE GUI 导入 `施工图\` 三件(dtlmodel+dtlCalc+Jccad_0), **导入时设自动生成筏板**, 保留完整上部结构, 存 .pre。
    - ⚠**命令流 `io.import_yjk` 不可靠, 完整模型走用户手动 GUI**: yjk_para 第8位=2"自动生成基础"实测**只建基础层、丢上部结构**(gset=8 只剩筏板); 第8位=0"读取基础"+真dtlCalc **0xC0000005 访问违规崩溃**——两值都不行。(位映射备查: 第8位基础模式 / 第13位筏板厚mm / 11筏板周长 / 14超出外轮廓 / 18维度3, 详见记忆。)
  - **步7 拿 .pre 做 GFE 后续**: 用户给 .pre 目录后 AI 接手做 B 段(土体/网格/SSI)。
  - *(旧版"命令流 import_yjk 一把梭 + yjk_exportydb 产 dtlCalc"已废: 堆损坏+丢上部结构, 见记忆纠错。)*
- **`GetJcXxxList()` 等返回 ByRef, pythonnet 抛 NotSupportedException → 必须走 `GetJcModelDBdata().ToPyList().m_jc*` 字段读基础模型数据**。
